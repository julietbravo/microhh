...
namespace
{
    ...
    void set_no_slip_new(double* const restrict ut, double* const restrict vt, double* const restrict wt,
                         ...
                         const int icells, const int ijcells)
    {
        ...
        for (std::vector<IB_cell>::iterator it=IB_cells.begin(); it<IB_cells.end(); ++it) // c++11 :(
        {
            ...
            // Adjust u-component directly north of cell
            if (it->sw_outer_wall[North_edge])
            {
                const int k   = it->k;
                const int ijk = it->i + (it->j+1)*jj + k*kk; 

                ut[ijk] +=
                        - ( rhorefh[k] * interp2(v[ijk-ii], v[ijk]) * interp2(u[ijk-jj], u[ijk]) ) / rhoref[k] * dzi[k]
                        + visc * ( (u[ijk] - u[ijk-jj]) * dzhi[k] ) * dzi[k]
                        - visc * ( 2.*u[ijk] * dzhi[k] ) * dzi[k];

                // For the top-right corner, also adjust u-component
                if(it->sw_outer_wall[East_edge])
                {
                    const int ijk = it->i+1 + (it->j+1)*jj + k*kk; 
...
void Immersed_boundary::init()
{
    ...
    const int patchsize_x = 16;
    const int blocksize_x = 8;

    const int patchsize_y = 16;
    const int blocksize_y = 8;
    
    const int npatches_x = grid->itot / patchsize_x;
    const int npatches_y = grid->jtot / patchsize_y;
    
    for (int i=0; i<npatches_x; ++i)
        for (int j=0; j<npatches_y; ++j)
        {
            const int i0 = (i+0)*patchsize_x + (patchsize_x-blocksize_x)/2;
            const int i1 = i0 + blocksize_x;
            const int j0 = (j+0)*patchsize_y + (patchsize_y-blocksize_y)/2;
            const int j1 = j0 + blocksize_y;

            add_IB_block(IB_cells, i0, i1, j0, j1, grid->kstart, grid->kstart+0.05*(grid->kend-grid->kstart));
        }
...
void Immersed_boundary::exec()
{
    ...
    set_no_slip_new(fields->ut->data, fields->vt->data, fields->wt->data,
                    fields->u->data, fields->v->data, fields->w->data,
